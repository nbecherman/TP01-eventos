...
router.post("/login", (request, response) => {
  ...
  if (typeof username === "string" && typeof password === "string") {
    ...
    const loginToken = UserService.postAutenticacionUser(username, password, token);
...
router.post("/register", (request, response) => {
  ...
  if (typeof first_name === "string" && typeof last_name === "string" && typeof username === "string" && typeof password === "string") {
    const registerUser = UserService1.postRegistrarseUser(first_name, last_name, username, password);
